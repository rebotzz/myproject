...
private:
    ...
    std::vector<std::shared_ptr<CollideBox>> objects_;      // 矩形区域的物体,碰撞盒子
...
private:
    ...
    std::vector<std::shared_ptr<CollideBox>> box_list_;    // 临时容器，准备下一次插入
...
private:
    ...
    // 智能指针，可能多个区域持有同一碰撞盒子,引用计数便于管理
    bool _insert(QuadTreeNode* node, const std::shared_ptr<CollideBox>& collide_box); 
